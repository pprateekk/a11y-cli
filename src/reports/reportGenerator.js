...
const {
  loadPreviousViolations,
  compareViolations,
} = require("./deltaReporter");
...
async function runAudit({
  ...
  try {
    ...
    if (comparisonFilePath) {
      const prev = loadPreviousViolations(comparisonFilePath);
      comparisonResult = compareViolations(filteredResults.violations, prev);
    ...
    let reportContent;
    if (format == "HTML") {
      reportContent = comparisonFilePath
        ? generateHTMLReport(filteredResults, url, severity, comparisonResult)
        : generateHTMLReport(filteredResults, url, severity);
    } else {
